<?php
	...
	class QDatabaseCodeGen extends QCodeGen {
		...
		public function GetTitle() {
			return sprintf('Database Index #%s (%s / %s / %s)', $this->intDatabaseIndex,
				(array_key_exists($this->intDatabaseIndex, QApplication::$ConnectionStringArray)) ? QApplication::$ConnectionStringArray[$this->intDatabaseIndex]['adapter'] : 'N/A',
				(array_key_exists($this->intDatabaseIndex, QApplication::$ConnectionStringArray)) ? QApplication::$ConnectionStringArray[$this->intDatabaseIndex]['server'] : 'N/A',
				(array_key_exists($this->intDatabaseIndex, QApplication::$ConnectionStringArray)) ? QApplication::$ConnectionStringArray[$this->intDatabaseIndex]['database'] : 'N/A'
			);
		}
		...
		public static function GenerateAggregateHelper($objCodeGenArray) {
			if (count($objCodeGenArray)) {
				...
				foreach ($objCodeGenArray as $objCodeGen) {
					$objTableArray = array_merge($objTableArray, $objCodeGen->TableArray);
				}
				...
				if (count($objForeignKey->ColumnNameArray) != 1)
					...
					if (array_key_exists(strtolower($strColumnName), $objTable->ColumnArray) &&
						...
						if (!$blnFound) {
							...
							array_push($objTable->IndexArray, $objIndex);
//							$objTable->IndexArray[count($objTable->IndexArray)] = $objIndex;
