	...
	abstract class QTextBoxBase extends QControl {
		...
		public function Validate() {
			...
			if ($this->intMinLength > 0) {
				if (strlen($this->strText) < $this->intMinLength) {
					if ($this->strName)
			...
			if ($this->intMaxLength > 0) {
				if (strlen($this->strText) > $this->intMinLength) {
					if ($this->strName)
						$this->strValidationError = sprintf($this->strLabelForTooLong, $this->strName, $this->intMinLength);
					else
						$this->strValidationError = sprintf($this->strLabelForTooLongUnnamed, $this->intMinLength);
					return false;
				}
			}
