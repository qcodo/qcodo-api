...
//			array_push($strToReturn, QRestServiceCodeGen::GenerateAggregateHelper($objRestServiceCodeGen));
		...
		public function GenerateFile($strTemplateFilePath, $mixArgumentArray, $blnSave = true) {		
			...
			$strError = 'Template\'s first line must be <template OverwriteFlag="boolean" TargetDirectory="string" TargetFileName="string"/>: ' . $strTemplateFilePath;
			...
			if (is_null($blnOverwriteFlag) || is_null($strTargetFileName) || is_null($strTargetDirectory))
				throw new Exception($strError);
