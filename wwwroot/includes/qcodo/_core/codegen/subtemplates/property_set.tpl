	...
	<% if ($objReverseReference->Unique) { %>
				case '<%= $objReverseReference->ObjectPropertyName %>':
					...
					if (is_null($mixValue)) {
						...
						// Make sure $mixValue is a SAVED <%= $objReverseReference->VariableType %> object
						if (<%= $objCodeGen->ImplodeObjectArray(' || ', '(is_null($mixValue->', '))', 'PropertyName', $objCodeGen->TableArray[strtolower($objReverseReference->Table)]->PrimaryKeyColumnArray) %>)
							throw new QCallerException('Unable to set an unsaved <%= $objReverseReference->ObjectMemberVariable %> for this <%= $objTable->ClassName %>');
