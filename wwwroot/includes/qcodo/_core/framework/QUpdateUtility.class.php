<?php
	...
	class QUpdateUtility {
		...
		public function RunUpdater($strInteractionType, $blnQuietMode) {
			...
			foreach ($this->objServerManifestDirectories as $objDirectoryToken)
				if (!defined($objDirectoryToken->Token))
					if (!$this->blnNewManifestPrefixWarning) {
						$this->strAlertArray[count($this->strAlertArray)] = 'Additional Directory Prefix Constants have been added to configuration.inc.php since the last time the QCodo Update Service was run.';
						$this->strAlertArray[count($this->strAlertArray)] = 'Be sure and download a new configuration.inc.php and update your local configuration.inc.php with any new Directory Prefix Constants.  Then re-run qcodo_updater.cli to complete the Qcodo update.';
			...
			foreach ($this->strServerManifest as $strFile => $strServerMd5) {
				...
				if ($strActualFilePath == -1) {
					...
					$this->strIgnoreArray[count($this->strIgnoreArray)] = $strFile;
		...
		protected function IgnoreFile($strActualFilePath, $strAction) {
			$this->strNoticeArray[count($this->strNoticeArray)] = sprintf('Ignored %s %s', $strAction, $strActualFilePath);
			print ("  Ignored\r\n");
		}
		...
		protected function SaveFile($strManifestFileString, $strActualFilePath, $strVerbPhrase, $strAlertMessage, $blnRename = false) {
			...
			if ($strSavedFile) {
				$this->strNoticeArray[count($this->strNoticeArray)] = $strVerbPhrase . ' ' . $strSavedFile;
				printf("  %s\r\n", $strAlertMessage);
			} else
		...
		protected function DeleteFile($strActualFilePath, $strReason) {
			...
			if ($blnSuccess) {
				$this->strNoticeArray[count($this->strNoticeArray)] = sprintf('Deleted %s %s', $strReason, $strActualFilePath);
				print ("  Deleted\r\n");
			} else
				print("  ERROR: See Update Log for more information\r\n");
		}
		...
		protected function AppendBaseSuffix($strActualFilePath, $strBaseSuffix, $strReason) {
			...
			if ($blnSuccess) {
				$this->strNoticeArray[count($this->strNoticeArray)] = sprintf('Renamed %s file to %s', $strReason, $strNewFilePath);
				print ("  Renamed\r\n");
			} else
				print("  ERROR: See Update Log for more information\r\n");
		}
		...
		protected function DownloadFile($strToken, $strFile, $strActualFilePath, $blnRename = false) {
			...
			if (strpos($strData, 'File Not Found: ') !== false) {
				$this->strAlertArray[count($this->strAlertArray)] = sprintf(
						...
						if (array_key_exists(strtolower($strFilePath), $this->strTouchedFile) &&
							...
							if (($blnIsCore) || (strpos($strFilePath, '/_core/') !== false)) {
								...
								if ($intReturnCode == 0) {
									...
									$this->strAlertArray[count($this->strAlertArray)] = 'Undecipherable issue with ' . $strFilePath . ' (a.k.a. ' . $strManifestFilePath . ')';
							}
...
', $strDetails);
	...
	function QUpdateUtilityErrorHandler() {
		QUpdateUtility::Error('Could not connect to Qcodo Update webservice at ' . QUpdateUtility::ServiceUrl);
	}
