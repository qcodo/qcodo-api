		...
		public static function InitializeDatabaseConnections() {
			...
			for ($intIndex = 0; $intIndex <= 9; $intIndex++) {
				...
				if (defined($strConstantName)) {
					...
					$objArray = unserialize($strSerialArray);
					
					if (count($objArray)) foreach ($objArray as $strKey => $mixValue)
						QApplication::$ConnectionStringArray[$intIndex][strtolower($strKey)] = $mixValue;
...
/*					$strNameValuePairs = explode(';', constant($strConstantName));
					...
					if (!QApplication::$ConnectionStringArray[$intIndex]['adapter'])
						throw new Exception('No Adapter Defined for ' . $strConstantName . ': ' . var_export($objArray, true));
					...
					$strDatabaseType = 'Q' . QApplication::$ConnectionStringArray[$intIndex]['adapter'] . 'Database';
