		...
		public function GetStyleAttributes() {
			...
			if (strlen(trim($this->strBorderWidth)) > 0) {
				...
				if (is_null($strBorderWidth))
					...
					$strToReturn .= sprintf('border:%spx;', $this->strBorderWidth);
...
/*		public function GetEndHtml() {
		...
		protected function RenderOutput($strOutput, $blnDisplayOutput, $blnForceAsBlockElement = false) {
			// First, let's mark this control as being rendered
...
//		abstract public function GetJavaScriptAction();
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case "FormAttributes": return $this->strFormAttributes;
		...
		public function __set($strName, $mixValue) {
			...
			switch ($strName) {
				...
				case "CausesValidation":
					try {
						$this->mixCausesValidation = QType::Cast($mixValue, QType::Integer);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
